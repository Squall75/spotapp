    ...
    <Box marginLeft="25px">
      ...
      <HStack spacing="5px" wrap="wrap">
        {albums?.items.map((albumDetails) => (
          <VStack w="128px" h="128px" onClick={() => setSelectedAlbum(albumDetails)}>

            <Image src={albumDetails.images[2].url} width="64px" height="64px" />
            <Link
              color={selectedAlbum?.id === albumDetails.id ? "gray.900" : "gray.600"}
              fontSize="x-small"
              key={albumDetails.id}
              width="64px"
              overflow="hidden"
